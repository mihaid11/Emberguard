...
BankMenu::BankMenu(sf::RenderWindow& window, int& crystals, int& storageCapacity,
    ...
    mTitle.setPosition(sf::Vector2f(mMenuShape.getPosition().x + (mMenuShape.getSize().x - mTitle.getScale().x) / 2.2
        , mMenuShape.getPosition().y + 15));

    sf::Vector2f buttonSize(155.0f, 72.0f);
    float gap = 50.0f;
    float startX = mMenuShape.getPosition().x + 50;
    float startY = mMenuShape.getPosition().y + (mMenuShape.getSize().y - 2 * buttonSize.x - gap) / 1.5;
...
int BankMenu::getRepaymentDay()
{
    return mBorrowMenu->getRepaymentDay();
}
...
void BankMenu::setBorrowStats(bool hasBorrowActive, int penalty, int interest, int amountToRepay, int repayDay)
{
    mBorrowMenu->setStats(hasBorrowActive, penalty, interest, amountToRepay, repayDay);
}
